...
const App: React.FC = () => {
  ...
  const [staffList, setStaffList] = useState<Staff[]>(STAFF_LIST); // Fetch from Supabase
      ...
      try {
        ...
        if (staffData && staffData.length > 0) {
            const formattedStaff: Staff[] = staffData.map((item: any) => ({
                id: item.id,
                name: item.name,
                role: item.phone, // Map phone to role
              ...
              <div className="flex flex-col min-w-full">
                      <div className="flex w-full sticky top-0 z-40 shadow-sm bg-slate-50">
                          ...
                          {daysArray.map(day => {
                              ...
                              return (
                                  <div key={day} className={`relative flex-1 min-w-[34px] md:min-w-[44px] flex flex-col items-center justify-center border-r border-b border-slate-200 py-2 ${showEmptyWarning ? 'bg-red-50 ring-1 ring-inset ring-red-200' : (isSpecial ? 'bg-slate-300' : '')} ${isCurrentDay ? 'bg-emerald-50 shadow-inner ring-2 ring-emerald-500 ring-inset z-10' : ''}`}>
                                      {isCurrentDay && (
                                          <div className="absolute -top-[1px] left-1/2 -translate-x-1/2 bg-emerald-600 text-white text-[8px] md:text-[10px] px-1.5 py-0.5 rounded-b-md shadow-sm z-20 font-bold whitespace-nowrap leading-none">
                                              วันนี้
                                          </div>
                                      )}
