...
export const OfficialPrintView: React.FC<OfficialPrintViewProps> = ({
  ...
  const daysArray = Array.from({ length: 31 }, (_, i) => i + 1); // Fixed 31 columns for consistency

  const getShiftCode = (staffId: string, day: number) => {
    // Hide data if day exceeds month length
  ...
  const calculateAmount = (staffId: string) => {
    ...
    for(let d = 1; d <= daysInMonth; d++) {
        ...
        if (shift) {
            ...
            // So Afternoon + Night = 750 (matches requirement)
  ...
  const PageLayout = ({ pageNumber }: { pageNumber: number }) => (
    <div className="print-page w-[297mm] h-[210mm] bg-white p-10 font-sans box-border relative text-black mb-10 overflow-hidden" style={{ color: '#000000' }}>
      
      {/* Header */}
      <div className="text-center mb-6">
        <h1 className="font-bold text-2xl mb-2 text-gray-900">หลักฐานการขออนุมัติปฏิบัติงานนอกเวลาราชการ</h1>
      ...
      <div className="w-full">
        <table className="print-table print-text-sm w-full">
            <thead>
                <tr className="h-14">
                <th style={{width: '40px'}}>ลำดับ<br/>ที่</th>
                <th style={{width: '180px'}}>ชื่อ – สกุล</th>
                <th colSpan={31} style={{padding: '4px'}}>วันที่ปฏิบัติงาน</th>
                <th style={{width: '120px'}}>{pageNumber === 2 ? "จำนวนเงิน" : "ลายเซ็น"}</th>
                </tr>
                <tr className="h-6">
            ...
            <tbody>
                ...
                <tr key={staff.id}>
                    <td>{index + 1}</td>
                    <td style={{textAlign: 'left', paddingLeft: '8px', fontWeight: '600', fontSize: '12px', whiteSpace: 'nowrap', overflow: 'hidden'}}>{staff.name}</td>
                    {daysArray.map(day => (
                    <td key={day} style={{
                        fontWeight: 'bold', 
